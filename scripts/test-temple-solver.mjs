...
for(const side of [-1,1]){
 ...
 for(const amount of [0,.25,.5,.8,1]){
  ...
  assert.ok(Math.abs(r.endX-hingeX)>=1.78*(.12+.08*amount)-1e-9);
  assert.ok(Math.abs(r.endX-hingeX)<=1.78*.38+1e-9);
  ...
  const expectedEar=side*1.08;
  assert.ok(Math.abs(r.endX-expectedEar)<=1.78*.38,"temple must stay bounded near detected ear");
 }
}
