...
for(const p of catalog.products||[]){
  ...
  const ok=aspect>=2.25 && aspect<=3.6 && topRatio<.16 && sel.asset?.frontTop>0;
  console.log(`${ok?"✓":"✗"} ${p.sku}: aspect=${aspect.toFixed(2)} top=${(topRatio*100).toFixed(1)}% crop=${sel.asset?.frontTop}`);
  if(!ok) failed=true;
}
